...
- =denote-tree-node-description=, elements of front matter to include in node's
  ...
  element, but they have to add corresponding regex to `denote-file-types' in
  order for denote-tree to recognize it.  That user variable also supports
  arbitrary strings.  Denote's default front matter elements: title, identifier,
