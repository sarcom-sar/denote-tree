...
paid once, at initialization.  After that =denote-tree= only redraws portions of
current tree, when you ask it to via =denote-tree-redraw= command.  It handle
both adding and deleting nodes, but sadly you have to do it yourself.  There are
(currently) no plans to do it from within the application.  As of right now,
there is on unsupported edge case while redrawing.  If you delete a node that is
"root" node with children and at the same time cyclical nodes inherit it's
children property.
